...
import { getDQMQualityRules, getDQMCurrentScore, createDQMQualityRule, updateDQMQualityRule, deleteDQMQualityRule } from '../api/client';
import { Modal } from '../components/common/Modal';
import { useStore } from '../store';

const DIM_TYPES = ['completeness', 'uniqueness', 'validity', 'consistency', 'timeliness', 'accuracy', 'integrity', 'conformity'];
...
export function DQMQualityRules() {
  ...
  const [newDimension, setNewDimension] = useState('completeness');
  ...
  const load = () => {
    setLoading(true);
    Promise.all([getDQMQualityRules(), getDQMCurrentScore().catch(() => null)])
  ...
  const handleCreate = async () => {
    ...
    try { await createDQMQualityRule({ name: newName.trim(), type: newType, dimension: newDimension, table: newTable, column: newColumn }); addNotification({ type: 'success', message: 'Quality rule created' }); setShowCreate(false); setNewName(''); load(); } catch { addNotification({ type: 'error', message: 'Failed to create rule' }); }
  };

  const handleDelete = async (id: string, name: string) => {
    if (!confirm(`Delete quality rule "${name}"?`)) return;
    try { await deleteDQMQualityRule(id); addNotification({ type: 'success', message: 'Rule deleted' }); load(); } catch { addNotification({ type: 'error', message: 'Failed to delete rule' }); }
  };

  const handleToggle = async (r: any) => {
    try { await updateDQMQualityRule(r.id, { enabled: !r.enabled }); addNotification({ type: 'success', message: `Rule ${r.enabled ? 'disabled' : 'enabled'}` }); load(); } catch { addNotification({ type: 'error', message: 'Failed to toggle rule' }); }
  };

  const openEdit = (r: any) => { setEditing(r); setEditName(r.name); setEditTable(r.table || ''); setEditColumn(r.column || ''); };
  const handleSaveEdit = async () => {
    if (!editing || !editName.trim()) return;
    try { await updateDQMQualityRule(editing.id, { name: editName.trim(), table: editTable, column: editColumn }); addNotification({ type: 'success', message: 'Rule updated' }); setEditing(null); load(); } catch { addNotification({ type: 'error', message: 'Failed to update rule' }); }
  };
    ...
    <div className="space-y-6">
      {score && (
        <div className="card p-6">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div><div className="font-semibold text-slate-900">Data Quality Score</div><div className="text-sm text-slate-500">{score.grade || ''} · {rules.length} rules</div></div>
          </div>
          {score.dimensions && <div className="grid grid-cols-4 gap-3">{Object.entries(score.dimensions).map(([k, v]: any) => (<div key={k} className="text-center p-2 bg-slate-50 rounded"><div className="text-lg font-semibold">{v ?? 0}%</div><div className="text-xs text-slate-500 capitalize">{k}</div></div>))}</div>}
        </div>
      )}

      <div className="flex items-center justify-between">
        <p className="text-sm text-slate-500">Define and manage data quality validation rules.</p>
          ...
          {rules.map((r) => (
            <div key={r.id} className="card px-6 py-4 flex items-center justify-between group">
              <div className="flex items-center gap-4">
                ...
                <div><div className="font-medium text-slate-900">{r.name}</div><div className="text-xs text-slate-500">{r.type} · {r.dimension} · {r.table || '—'}.{r.column || '—'}</div></div>
              </div>
      ...
      <Modal open={showCreate} onClose={() => setShowCreate(false)} title="Create Quality Rule">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4"><div><label className="label">Type</label><select className="input" value={newType} onChange={e => setNewType(e.target.value)}>{DIM_TYPES.map(t => <option key={t} value={t}>{t}</option>)}</select></div><div><label className="label">Dimension</label><select className="input" value={newDimension} onChange={e => setNewDimension(e.target.value)}>{DIM_TYPES.map(t => <option key={t} value={t}>{t}</option>)}</select></div></div>
